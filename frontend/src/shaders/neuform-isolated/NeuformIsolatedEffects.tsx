...
/* the headline builder, replaced by an empty layer so only the rotating 3D gallery plates are rendered */
...
function transformGalleryHeadingSource(
  ...
  return source
    ...
    .replace(/background:\s*(#[0-9a-fA-F]+|transparent)/g, "background:#FFFFFF")
    .replace("axis: 25.5,", `axis: ${variant.axis},`)
    .replace("phase: 93", `phase: ${variant.phase}`)
    .replace("  a: 712,", "  a: 1020,")

    /* 4:3 tiles: enlarged tile side and semi-major axis for bigger 3D rotating presentation */
    .replace(
      "  tile: 346,            /* tile side in ring units (R = a)               */",
      "  tile: 480,            /* tile width in ring units (R = a)              */\n"
    ...
    .replace(
      ...
      `var HEAD_CAP = 142;\n`
        + `var HEAD_MID = 1093;\n`
        + `var HEAD_SIZE = ${variant.type.headlineSize};\n`
        + `var HEAD_WEIGHT = '${variant.type.weight}';\n`
        + `var HEAD_TRACK = ${variant.type.tracking};\n`
        + `var HEAD_STYLE = '${variant.field}';\n`
        + `var HEAD_SHADOW = 'transparent';\n`
        + `var HEAD_GHOST = ['transparent','transparent'];\n`
        + `var PLATES = ${JSON.stringify(variant.palette)};\n`
        + `var FIELD = '${variant.field}';\n`
        + `var EASE = ${variant.motion.ease};\n`
    ...
    .replace("{ s:'NEW GRAINIENT',    top:930,  w:1370, fill:'#d0d0d0' }", `{ s:'', top:930,  w:1, fill:'transparent' }`)
    .replace("{ s:'COLLECTION ADDED', top:1114, w:1775, fill:'#ffffff' }", `{ s:'', top:1114, w:1, fill:'transparent' }`)
