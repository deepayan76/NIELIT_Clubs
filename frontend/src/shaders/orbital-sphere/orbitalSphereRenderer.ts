...
export function createOrbitalSphereRenderer(canvas: HTMLCanvasElement, getOptions: () => OrbitalSphereOptions) {
  ...
  const particleCount = Math.max(3500, Math.round(baseCount * getParticleScale()));
  ...
  for (let index = 0; index < 6; index += 1) {
    ...
    if (index % 2 !== 0) {
      ...
      const haloMaterial = new THREE.MeshBasicMaterial({ color: 0xc084fc, transparent: true, opacity: 0.2, blending: THREE.AdditiveBlending });
  ...
  return {
    ...
    render() {
      ...
      networkGroup.children.forEach((child: THREE.Object3D, index: number) => {
